...
\begin{equation}
	...
	\begin{pmatrix} S_o G & S_o & -To \\ T_i & K S_o & K S_o \end{pmatrix}
	\begin{bmatrix} d_u \\ d_y \\ n \end{bmatrix}
\end{equation}
where $S_o = (I + G K)^{-1}$ is the output sensitivity, $T_o = (I + G K)^{-1} G K = S_o G K$ is the output complementary sensitivity, and $T_i = (I + K G)^{-1} K G$ is the input complementary sensitivity.

The output sensitivity is of particular importance since it is a factor in most of these transfer functions.
We choose the upper bound of $S_o$ such as to guarantee a bandwidth of 30\,Hz and a maximum peak of 1.9 (which guarantees a stability margin of 0.53).
In the control literature, a maximum peak sensitivity of 2 or less is considered good for most intents and purposes.
We also want the output (respectively input) complimentary sensitivity to start rolling off at 200\,Hz maximum, have a maximum 20\% (respectively 30\%) amplification at low frequencies.
We want $S_o G$ to roll-off at no more than 50\,Hz and have at least a 14\,dB attenuation.
To limit actuator use, we want $K S_o$ to have a bandwidth of at least 10\,Hz and no more than 40\,dB amplification.
...
\begin{figure}[htbp]
	...
	\includegraphics[width=0.5\linewidth]{chap10/robust_performance_Ti}%
	\includegraphics[width=0.5\linewidth]{chap10/robust_performance_SoG}
	\includegraphics[width=0.5\linewidth]{chap10/robust_performance_KSo}
